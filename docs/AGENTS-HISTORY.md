...
## 18. «Hostile-Critic Loop» — Циклы 38–42 (23.08.2026, Z.ai Agent)

Метод: 5 циклов «3 враждебных критика (Task-субагенты) → исправить ВСЕ дефекты
→ commit+push → wait 120s → клик-верификация → новые критики». Промпты критиков
в чате-оркестраторе; worklog: /home/z/my-project/worklog.md (Tasks 38–42).

**Прогресс оценок:** 5.0–6.0 → 5.5–6.5 → 4.6–6.4 → 3.8–5.6 (фризы) → **ACCEPT 8.0** (цикл 5).
125+ дефектов исправлено, коммиты `06d8934` → `f6bc56f`.

### Ключевые исправления (по важности)

1. **PERF (критично):** GrainOverlay анимировал `background-position` на
   `mix-blend-mode: overlay` full-viewport слое → некомпозитабельный full-page
   repaint 20×/с → 100% фриз main thread через 10–60с. Фикс: статичное зерно.
   gamma-marquee: GSAP-твин → CSS `@keyframes` (композитор). УРОК: только
   transform/opacity, никаких background-position + blend-mode анимаций.
2. **КОНВЕРСИЯ (критично):** /api/lead отвергал «сырые» 10-значные телефоны
   (400) → normalizePhone() клиент + расширенный regex API. Голый ввод
   «9991234567» → 201 (проверено на проде).
3. **Видео:** 6 <video> с одним 5MB src душили пул соединений (lazy-картинки
   висели «битыми») → 1 видео + постеры; карусель: 4 уникальных ffmpeg-клипа
   (нарезка mculinary-hero) вместо одной и той же лжи.
4. **Trust:** индекс 191186 (Почта России; 191181 из битого сниппета Яндекса),
   координаты 59.934862/30.316218; ОГРНИП чексумма; FAQ=оферта=JSON-LD;
   год 2009/17 лет; 120 000 гостей; «Лучший»→«Премиальный» (ФЗ о рекламе).
5. **A11y:** RU aria-labels везде; focus-trap + focus-return в модалке;
   roving tabindex табов меню; ответы FAQ в SSR (grid-rows 0fr→1fr вместо
   AnimatePresence unmount); тач-таргеты 44px; шрифты ≥12px.
6. **UX:** калькулятор → форма (CustomEvent + URL-параметры fallback);
   расчёт едет в payload лида; услуги префиллят тип; success-экран держится
   до явного клика; cookie-баннер снизу, компактный, FAB смещается.

### Грабли (новые)

- `useQueryState` дефолты менять осторожно (банquet→buffet синхронизирован
  с меню-нарративом).
- ESC-обработчик в модалке обязан идти через тот же closeModal, что и клик
  (иначе focus-return не срабатывает).
- Web Audio: AudioContext нужен `ctx.resume()` внутри жеста клика.
- Критики-субагенты на headless Chrome: НЕ анимируйте некомпозитабельно —
  весь сайт «умирает» и критики ставят 3.8 за «мёртвый интерактив».
- VLM-скриншоты врут про «обрезанные табы» — проверяйте кропом перед фиксом.

### Что осталось (цикл 6+)

- Цель: все критики ≥9.3 (сейчас 8.0 у одного комбинированного).
...
## 20. Cycle 44 — блок услуг «Сцена»: August-2026 awwwards meta (24.08.2026, Z.ai Agent)

Пользователь отклонил Cycle 43 («не awwwards уровень на август 2026,
устаревшее»). Исследование живых данных SOTD/SOTM августа 2026 подтвердило:
курсор-карточка Dennis — коммодитизированный паттерн 2021–23 (Webflow-
туториалы с 2025), пунктирные лидеры — хром САМОГО awwwards.com (ни один
победитель их не использует), Inter Tight — шрифт-по-умолчанию, #F8F8F8 —
холодный стерильный серый. Замена: StageServices («Сцена»).

### Грабли цикла 44 (новые)

1. **VLM судит СТАТИЧНЫЕ скриншоты по ДИНАМИЧЕСКИМ эффектам** — «зерно не
   шевелится», «variable font не дышит», «marquee не движется» — всё это
   ложные срабатывания. Перед фиксом такого «дефекта» ВСЕГДА проверяй
   computed styles live: `animationName`, `fontVariationSettings` на hover.
   В R1 из 19 «дефектов» 11 были такими фантомами.
2. **Реальный reduced-motion тест = `set media reduced-motion` + СВЕЖАЯ
   НАВИГАЦИЯ** (`open URL`, не reload — reload сбрасывает эммуляцию).
   Подтверждён путь: canvasCount 0 + статичная статистика. НЮАНС: CDP-эмуляция
   НЕ стреляет change-event → framer `useReducedMotion` НЕ ре-рендерится при
   live-переключении (у реального юзера с ОС-настройкой медиа-квери читается
   при первом рендере — путь работает). Поэтому все live-флипающиеся
   фолбэки делай CSS-@media (спотлайт, зерно, marquee, hooks), а React-ветки
   (canvas/одометры) рассчитывай на load-time.
3. **ogl**: uniform flowmap передаётся как `flowmap.uniform` (живой ref,
   класс меняет текстуры внутри) — НЕ оборачивать в `{ value: ... }`.
   `velocity.needsUpdate` из примеров OGL отсутствует в TS-типах — юзай
   локальный boolean `hasNewPointer`.
4. **JSX многострочные строковые литералы** `{"...\n..."}` ломают парсер —
   русский копирайт в одну строку.
5. **Entrance-burst трюк** (для displacement-шейдеров): та же текстура в
   оба слота + progress=0 → чистая жидкостная рябь без смены картинки.
6. **ogl + React 19**: императивный canvas в useEffect, никакие обёртки
   не нужны; `renderer.setSize` + ResizeObserver; cleanup через
   `WEBGL_lose_context.loseContext()` + `gl.canvas.remove()`.
...
## 24. Cycle 51 — filmstrip pass: закрытый стеллаж становится кино (24.08.2026, Z.ai Agent)

> Второй вызов пользователя «то есть сейчас это лучшее, на что ты способна?».
> Оба цикла критики сходились на одном: закрытые корешки = «образцы краски».
> Коммит `06e0a43`.

**Суть:** каждый item несёт своё фото edge-to-edge за всем стеком
(декоративный дубль с alt=\"\"), закрытым — клипится до ширины корешка:
rack = 12 тёмных вуалированных кадров с белой вертикальной типографикой.
Открытие «проявляет» кадр в светлую тинтовую панель (тёмное→светлое).
Тело панели — непрозрачная тинтовая карточка над фото-слоем → открытый
корешок сохраняет фото («корешок книги = обложка»).

Ключевые правки:
- **Активное состояние неотразимо**: открытый корешок = шире + вуаль 0.58 +
  ПОСТОЯННАЯ красная линия + красный номер (VLM: «считывается мгновенно»);
  hover закрытого = вуаль светлеет + линия + подъём тайтла
- **Бесконечный Ken Burns дрейф** (34s alternate, transform-only) вместо
  one-shot выдоха + унифицирующий цветокор (saturate 0.94/contrast 1.05)
  всех 12 фото
- **A11y**: фокус-контур белый (на тёмном), forced-colors роняет filmstrip
  (CanvasText-типографика наplain фоне), print прячет фото-слой,
  reduced-motion гасит вуаль-переходы + дрейф

### Грабли цикла
- **Static-контент тонет под positioned z0**: на мобиле spine был
  `position: static` → его типографика ушла ПОД фото-слой (absolute z0).
  Ин-флоу непозиционированные элементы красятся НИЖЕ позиционированных.
  Фикс: `position: relative; z-index: 2` на мобиле.
- **Один next/image src → два варианта**: фото-слой корешка (sizes 96px,
  слайс ~3-5KB webp) + медиа-фигура (sizes 62vw) — браузер качает оба,
  это дёшево (~60KB на 12 панелей).
- **Вуаль для AA на фото**: плоская rgba недостаточна (яркие фото убивают
  белый текст) → градиент 0.68/0.56 + text-shadow 14px; worst-case
  bright-photo lum 0.9 → 0.9·0.44 ≈ 0.4 → белый 4.6:1 ✓.
- **Дубль hover-правил после переноса блоков** — проверять rg-ом на
  `spine:hover` после перестановки CSS-блоков.

### Верификация
12/12 фото-слоёв грузятся; parallax −5.96px и hover-intent (idx 5)
работают поверх filmstrip (Playwright); мобайл: бары 84px с белым Marck
тайтлом на фото; JS errors NONE; lint/typecheck зелёные. VLM: эмоция 9/10,
запоминаемость 9/10, «лучше для премиума».

### Динамика качества секции
Cycle 49: 5.5 → 8 → 8.5 (gamma-механика + баги-фиксы) → Cycle 50: alive
(ambient/counter/parallax/magnetic) → Cycle 51: filmstrip (кинематографика
закрытого состояния + драматургия проявления).

---

## 25. Cycle 52 — две стойки по шесть: финальная форма по вердикту юзера (25.08.2026, Z.ai Agent)

> Вердикт юзера после cycle 51: «мне больше нравилось, когда каждый корешок
> был однотонного цвета как до этого; еще корешков слишком много, может
> разделить их пополам и сделать просто две секции, вторую ниже». Коммит
> `411eb5d`. Юзер также снял жёсткость правила §5: transform/opacity-only
> МОЖНО нарушать, если это делает сайт лучше (пока не потребовалось —
> flex/grid-анимации уже «нарушают» букву правила ради gamma-механики).

**Итоговая архитектура:**
- Rack A «01–06 · Форматы события» (фуршеты→барбекю) + Rack B
  «07–12 · Сервисы и дополнения» (бар→логистика) — СМЫСЛОВОЕ деление, не
  механическое. Каждая стойка — полный gamma-haccordion.
- 6 корешков вместо 11 у открытой панели → панель 71% ширины стойки
  (gamma-уровень; 12-корешковая стойка давала только 67%). Корешки снова
  щедрые: 56→96px.
- Filmstrip cycle-51 ОТКАЧЕН по вердикту юзера: однотонные тинты,
  ink-типографика, red-deep активное состояние, 5%-затемнение hover.
  Оставлено из cycle 51: бесконечный Ken Burns дрейф + унифицирующий
  цветокор.
- Метки групп: eyebrow «01–06 Форматы события» + hairline, в ea-container.

**Модель фокуса двух стоек (единственная новая механика):**
- каждая стойка держит свой openIndex — открытие в B не трогает A;
- автоплеит ТОЛЬКО сфокусированная стойка (premium restraint): фокус
  следует за hover/клик/клавиатурой (engage = фокус + стоп автоплея,
  WCAG 2.2.2) и за скроллом (IO ratio-гистерезис 0.15; ручной фокус
  «липкий» 2.5с — скролл мимо читаемой стойки не крадёт фокус);
- ambient wash + счётчик HUD зеркалят открытую панель сфокусированной
  стойки (глобальный индекс 01–12, `openByRack[focusedRack]`).

### Грабли цикла
1. **React Compiler требует полные deps**: `open()` читал `N`
   (items.length) при deps [reduced, onEngageRack] →
   `react-hooks/preserve-manual-memoization` ERROR. Фикс: добавить N в deps.
2. **AnimatePresence popLayout держит exiting-span в DOM** до конца
   exit-анимации: `querySelector('.hacc__counter-num')` во время перехода
   возвращает СТАРЫЙ span → тесты «счётчик не обновился» = ложная тревога.
   Верифицировать через ≥1.5с после события или по последнему span.
3. **Inline-стрелочные пропсы (makeOnOpen) меняют identity каждый рендер
   секции** → rack-эффект отчёта перезапускается — безвредно (guarded
   setState no-op), но держать в голове при дебаге эффектов.

### Верификация
Панель 71% в обеих стойках; независимость (A открыта при клике в B);
фокус+счётчик+ambient переключаются (автоплей тик 07→08 в B при
замороженной A); скролл-передача фокуса; мобильный стек двух групп с
метками; Playwright: hover-intent в обеих стойках, параллакс ±8px,
magnetic −32px; ошибок консоли нет; lint/typecheck зелёные.
VLM: 9/10, production-ready (его «mobile weakness» — галлюцинация
статичного десктоп-кадра; мобильная версия проверена измерениями).

### Статус секции услуг
Циклы 49→52: gamma-механика + фиксы багов gamma → alive-слой → filmstrip
(откачен по вердикту юзера) → **две стойки по 6, однотонные тинты —
финальная форма**. Дальнейшие улучшения = контент (реальная съёмка
вместо gamma-фото, когда появится возможность).

---

## 26. Cycle 53 — фото перестали врать, копирайт задышал (25.08.2026, Z.ai Agent)

> Вызов юзера: «независимые критики любой модели поставят 10/10? все фото и
> слова идеально подобраны?» Слепой критик (без истории проекта) вынес
> вердикт 6.5/10: главный провал — фотослой: 7/12 панелей семантически
> врали (пустой зал у «Шоу-станций», лаундж у «бара», паэлья у «барбекю»,
> цветы у «свадьбы», 5 ложных alt). Коммит `d5def5d`.

**Замена 7 фото** (image-search skill → contact-sheet → VLM-отбор по
критериям семантика+премиальность+тёплый свет+горизонталь):
свадьба (банкет при свечах), барбекю (рибай на гриле — после первого
 mismAtch-отбора), кофе-брейк (стойка с выпечкой), бар (бармен наливает),
шоу-станции (шеф в облаке пара), боксы, торт (2 яруса + ягоды). Все
w1200 AVIF-тесты <1.2s (нет зависаний), webp q82, честные alt.

**Копирайт-пасс** (второй слепой критик: «9 из 12 хуков — паспорт стола»):
H2 → «Сначала формат. Потом меню.»; хуки переписаны сенсорно
(«Канапе — тёплыми, коктейли — ледяными…», «Шейкер звенит, бокалы
ледяные…», «Выпечка ещё тёплая, кофе пахнет на весь этаж»); теги
без жаргона («Базовая подача»→«Классическая», «Live cooking»→«Живая
кухня», «Под ключ»→«Всё, кроме еды»); CTA-глаголы унифицированы.

**Дизайн-фиксы:** CTA → --ea-red-deep (AA 5.9:1, было 3.5–3.9);
хук 14.4→16.5px (продающая строка была мельче цены); цена первой,
подпись под ней; Marck Script → Neucha (аккуратное перо против
школьных петель).

### Грабли цикла
1. **Замена файла по тому же URL = кэш-ловушка**: браузер (и Next
   optimizer) отдают СТАРУЮ картину по прежнему URL даже после dev-перезагрузки.
   Фикс: переименовать файл (ribay.webp) — гарантированный bust.
2. **MultiEdit не всегда атомарен на практике**: часть edits применяется
   до ошибки — ВСЕГДА проверять rg-ом фактическое состояние после
   «failed» MultiEdit, не предполагать «ничего не применилось».
3. **VLM-подбор фото по контакт-листу ≠ проверка в панели**: контактный
   лист кадрирует иначе, чем object-fit:cover в панели (барбекю-«рибай»
   в листе оказался овощами в панели). Двойная проверка: лист → скрин
   панели → при mismatch искать замену.
4. **Bash read с IFS=: ломается на строках с двоеточиями внутри** —
   использовать | как делимитер в спеках.

### Итог
VLM-гейтенлет: фото 9/10 (было «тройка» у слепого критика), стойки 8/8,
мобайл 9/10; Playwright: hover-intent/параллакс/автоплей целы; ошибок 0.
Честный потолок заявлен юзеру: «10/10 от любой модели» недостижимо
верифицируемо (VLM галлюцинирует и варьирует), но все НАЙДЕННЫЕ
реальными критиками дефекты закрыты; следующий уровень = только
реальная съёмка.

---

## 27. Cycle 54 — дефекты, пойманные юзером (25.08.2026, Z.ai Agent)

> Юзер поймал 4 реальных провала, пропущенных моей «верификацией». Коммит
> `d7cbb8a`. Уроки — в граблях.

1. **Барбекю = инфографика** (колесо температур, крупные надписи). Мой VLM-чек
   спрашивал «есть ли стейк?» — стейк был, но чек не спрашивал «есть ли
   ГРАФИКА/текст». Заменено: рибай на живых углях (чистый кадр).
2. **Кофе-брейк = вотермарки alamy** по всему кадру. Заменено: кофейный стол
   с круассанами и латте-артом 3840px (чистый).
3. **Слова ломались посреди слова** («Вегетариан-ское»): `overflow-wrap:
   anywhere` + `max-width: 68%` сжимали длинные титулы в 3 рваные строки.
   Убраны оба → «Вегетарианское и халяль» на ОДНОЙ строке (проверка
   clientHeight — не bounding rect, который раздувается rotate).
4. **Шрифт возвращён на Marck Script** (Neucha не зашёл юзеру), импорт
   Neucha удалён из layout.
5. **Счётчик 01/12 удалён** — тикал в 700px от взгляда при чтении стойки B,
   дублировал индексы корешков; декорация без работы (слепой критик говорил
   то же). Мёртвый CSS вычищен.

### Грабли цикла (КРИТИЧНО для будущих проверок)
1. **VLM-верификация foto должна спрашивать о ЗАПРЕЩЁННОМ явно**: «есть ли
   текст/вотермарки/логотипы/инфографика/схемы?» — вопрос «что на фото»
   находит контент и ПРОПУСКАЕТ наложения. Строгий скан 12 фото нашёл ещё
   alamy-вотермарки, которые «MATCH=yes»-проверка благословила.
2. **bounding rect врёт для повёрнутых элементов**: rotate(-6°) раздувает
   height → «3 строки» при одной реальной. Считать строки только через
   clientHeight (layout-высота, трансформ не входит).
3. **overflow-wrap: anywhere на заголовках — мина**: ломает слова посреди
   слова в узких грид-колонках. Только normal + перенос по пробелам.
4. **max-width в % от грид-колонки double-squeeze**: колонка auto уже
   сжимается фланками tag/price — процент от неё добивает титулы. Ширину
   титула ограничивает сама сетка.
5. **Декоративные HUD-цифры без работы — удалять сразу**: если элемент не
   отвечает на вопрос «что он делает для читателя СЕЙЧАС» — он мусор.
