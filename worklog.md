...
---
Task ID: c102-admin-frontend
Agent: full-stack-developer
Task: админка-мини-CRM (статусы/кнопки связи/заметки/фильтры/CSV) + карточка «Автоматизация» + mock-слой

Work Log:
- СТАРТ: изменения c102 уже лежали в рабочем дереве (прерванный запуск той же
  задачи — код есть, верификации и worklog не было). Сессия сверила реализацию
  со спекой ПОСТРОЧНО (api/leads-view/admin-app/settings-view) и добила
  полный верификационный прогон.
- api.ts: Lead += status/note/followupTs/followupOk/nudgedTs; AdminSettings
  += autoDigest/autoFollowup/autoNudge/autoNudgeHours/cronEnabled/cronState;
  SettingsPatch += те же поля + clearCronToken (cronEnabled/cronState
  read-only — в патч НЕ отправляются); apiLeadUpdate patch расширен
  {read,archived,status,note}; НОВЫЙ apiCronToken(revoke) → POST cron-token
  ({revoke:true} → token:null) — токен приходит ровно один раз.
- api.ts MOCK: mockCronToken в памяти модуля (как secrets.json на проде);
  action cron-token (генерация 32-hex / revoke); settings-POST применяет
  autoDigest/autoFollowup/autoNudge/autoNudgeHours(1-24, иначе 400)+
  clearCronToken; lead-update мержит status (whitelist 4 значений) и note
  (null/""→очистка, \r\n→\n, >1000→400); mockSettings дефолты: сводка/дожим
  вкл, пинг выкл/3ч, cronEnabled:false, cronState{lastRunTs-900с, runs:42};
  mockLeads: Игорь=contacted+note, Мария=agreed, Ольга=followupTs+followupOk
  +nudgedTs (демо авто-маркеров), телефон «8…» — на нормализацию ссылок.
- leads-view.tsx: воронка НОВАЯ→СВЯЗАЛИСЬ→ДОГОВОРИЛИСЬ→ОТКАЗ сегментом из
  4 кнопок в карточке (активная подсвечена: золото/ink/moss/bordeaux — без
  синего); статус≠new шлёт read:true тем же патчем; ссылки tel/t.me/+цифры/
  wa.me/цифры рядом с телефоном (phoneDigits: 8→7, 10 цифр→7, прочее→null
  — кнопок нет вовсе) — клик по связи при статусе new/undefined авансом
  двигает {read:true,status:"contacted"} fire-and-forget, БЕЗ
  preventDefault (переход в мессенджер не ломаем); копирование email
  (паттерн copyPhone); сворачиваемая «Заметка» (StickyNote): textarea+
  Сохранить→onPatch(id,{note}), сохранённый текст break-words
  whitespace-pre-wrap; бейджи followupTs «✉️ авто-дожим отправлен» /
  nudgedTs «🔔 напоминание было»; архив toggle (Archive/ArchiveRestore) —
  карточка opacity-60 + бейдж «в архиве»; 6 табов-фильтров (Все/Новые(+count)
  /В работе/Договорились/Отказ/Архив, flex-wrap) + пустые состояния с
  подсказкой следующего шага воронки; строка статистики «Всего N · сегодня
  N · за 7 дней N · в работе N · договорились N» (в работе/договорились —
  по неархивным); CSV += колонки «Статус» (русский ярлык) и «Заметка»
  (переводы строк→пробелы), RFC-4180 escaping сохранён; длинные
  comment/note/email — break-words/anywhere.
- admin-app.tsx: patchLead расширен {read,archived,status,note} —
  оптимистично + откат при сбое (идиома c96 сохранена); LeadsView получает
  расширенный onPatch.
- settings-view.tsx: карточка «Автоматизация» ПОСЛЕ существующих (тот же
  идиом Card/SectionHeader): 3 Switch (сводка / авто-дожим с описанием
  «через 24 ч одно письмо с меню и кнопками связи» / пинг о зависших) +
  NumberField «Часов до напоминания» 1-24 (виден только при вкл. пинге);
  сохранение СВОИМ патчем через общий apiSaveSettings — другие карточки
  автоматизацию не затирают; блок «Планировщик хостинга»: !cronEnabled →
  подсказка + «Создать ссылку для планировщика» → apiCronToken() →
  readonly-инпут https://nilovcatering.ru/api/cron.php?token=<32hex> +
  «Скопировать» + предупреждение «показывается один раз… каждые 15–30
  минут», onSettingsChange(cronEnabled:true); cronEnabled → «Планировщик
  подключён ✓» + «Создать заново»/«Отключить» (revoke → toast); details
  «CLI-вариант для cron хостинга: /usr/bin/php /home/<аккаунт>/site/
  public/api/cron.php <токен>»; блок «Состояние» из cronState (unix-СЕК,
  Europe/Moscow): Последний запуск / Запусков всего / Последняя сводка,
  пусто → «Ещё ни одного запуска»; revealedToken живёт ТОЛЬКО в локальном
  state (не в settings/патчах — сервер токен больше не отдаёт).
- ВЕРИФИКАЦИЯ: bun run lint — чисто; bunx tsc --noEmit — чисто.
- БРАУЗЕР (agent-browser, http://localhost:3001/admin?mock=1, вход admin):
  статус-кнопки переключают состояние (Анна: Новая→Связались, табы
  пересчитались 3→2/1→2, карточка стала read); фильтры: «В работе»=только
  contacted, «Архив»=пустое состояние; заметка: текст→Сохранить→
  «Заметка · есть» + текст в карточке; ссылки: t.me/+79218451230,
  wa.me/79218451230, «8 (921) 555-04-88»→t.me/+79215550488 (нормализация
  8→7 работает); клик TG-ссылки на «Новой» заявке Ольги → открылся t.me
  (новая вкладка) И заявка авансом стала «Связались»+прочитано —
  fire-and-forget подтверждён; архив: карточка opacity 0.6 + бейдж
  «в архиве», «Вернуть из архива» возвращает; Настройки→«Автоматизация»:
  свитч пинга показывает поле часов (3), «Создать ссылку» раскрыла URL
  cron.php?token=8ad2f563…+«Скопировать», «Отключить»→toast+кнопка
  «Создать ссылку» вернулась, «Сохранить»→toast «Автоматизация
  сохранена»; Состояние: «Последний запуск: 21 сент., 06:00 по Москве ·
  Запусков всего: 42»; CLI-хинт в DOM; консоль БЕЗ ошибок (только HMR);
  390px: Заявки и Настройки scrollW=390, карточек-переливов 0.
  Скриншоты: research/c102/admin-leads-desktop.png,
  admin-leads-mobile-390.png, admin-settings-mobile-390.png,
  admin-automation-mobile-390.png.

Stage Summary:
- Мини-CRM в «Заявках»: воронка 4 статуса одним тапом, авто-двигатель
  «связались» при первом клике tel/TG/WA, заметки, архив, 6 фильтров,
  статистика, статус+заметка в CSV — всё на оптимистичном onPatch с откатом.
- Карточка «Автоматизация»: 3 правила + секретная ссылка cron.php с
  показом токена ровно один раз (генерация/перегенерация/отзыв), read-only
  «Состояние» планировщика, CLI-подсказка; сохранение общим патч-механизмом
  без затирания чужих полей.
- MOCK-слой полноценно эмулирует cron-token/статусы/заметки — демо на
  ?mock=1 выглядит как прод. lint+tsc чисто, браузер e2e зелёный,
  390px без горизонтального переполнения. Файлы: src/components/admin/
  {api.ts, leads-view.tsx, admin-app.tsx, settings-view.tsx}.

---
Task ID: c102 (главная сессия)
Agent: main
Task: (1) TG-кнопка «Написать в Telegram» первой (чат с клиентом по номеру); (2) MAX-мессенджер во всех контактах (сайт+письма); (3) аудит переносов строк; (4) админка-мини-CRM + Автоматизация (cron.php: дайджест/дожим/напоминания) — «поиск клиентов без ручных действий»

Work Log:
- ИССЛЕДОВАНИЕ MAX: web-search + живые запросы max.ru → профиль max.ru/u/f9LH… (уже в config c92) и share-формат https://max.ru/share?url=…&text=… (200, «Перейти в чат», параметры сохраняются) — годен для кнопки «Поделиться».
- lead.php: lead_phone_digits() (нормализация 8→7/10→7, мусор→null) + lead_tg_href() (t.me/+<номер>) + lead_tg_buttons() в ДВЕ строки: [✈️ Написать в Telegram][💬 WhatsApp] / [📋 Меню клиента] — Telegram первым (просьба владельца), WA остался запасным каналом; комментариев/docblock обновлены.
- Письма: _lib.php OWNER_* константы (телефон/TG-чат/WA/MAX-профиль+телефон); mail_html_wrap подвал = Телефон + «Написать: Telegram · WhatsApp · MAX (+7 911 826-39-26)» + Сайт — MAX во ВСЕХ HTML-письмах автоматически; Telegram в письмах теперь ПРЯМОЙ ЧАТ t.me/+79119417205 (был канал); lead_client_mail_text: + «Макс (мессенджер MAX): …» и «написать в MAX: <url>» (перенос-safe).
- Сайт: site-header дровер + третья плитка «Макс» (col-span-2 на мобиле, глиф-MAX как в hacc-booking); contact.tsx (legacy-файл, конвенция репо) + MAX-карточка в «Другие способы связи»; calculator.tsx (legacy) + MAX share. ЖИВАЯ главная: hacc-booking и футер MAX уже имели (c89/c92) — итого MAX во всех видимых контактах.
- АДМИН-БЭКЕНД: admin.php lead-update +status(new|contacted|agreed|lost)+note(≤1000, CRLF→LF); settings +autoDigest/autoFollowup/autoNudge/autoNudgeHours(1-24)/clearCronToken, GET +cronEnabled+cronState (токен НЕ отдаётся); НОВЫЙ action=cron-token (32 hex random_bytes, revoke). _lib.php: дефолты+нормализация настроек, cron_state_path().
- cron.php (НОВЫЙ): GET ?token= / CLI argv; hash_equals + 403-без-различия + usleep анти-перебор; ДАЙДЖЕСТ (первый запуск после 09:00 МСК, раз в день, TG+почта, пусто→молча); АВТО-ДОЖИМ (24 ч, статус «новая», есть email → ОДНО письмо с тем же PDF-меню + кнопки-пилюли TG/WA/MAX + копия запроса, followupTs в лид, ≤5/запуск); НАПОМИНАНИЕ TG (N часов, 9-21 МСК, ≤3/запуск, выкл по умолчанию); состояние cron-state.json; все письма через mail_send + mail-log (cron-digest/lead-followup); include lead.php под NILOV_LEAD_TESTS (двойной include _lib — найден тестами и исправлен).
- АДМИН-ФРОНТЕНД: делегирован full-stack-агенту (c102-admin-frontend) — статусы-сегмент, быстрые ссылки tel/TG/WA с авто-«Связались» при клике, копия email, заметки, бейджи дожима/напоминания, архив, 6 фильтров, статистика, CSV+Статус/Заметка, карточка «Автоматизация» (тумблеры/часы/ссылка-токен с показом один раз/состояние/CLI-подсказка), mock-слой.
- ПЕРЕНОСЫ СТРОК: браузерный аудит 320px (docW==vw, 0 переполнений; marquee/слайдер-тики = намеренные) + VLM-проверка контактов/футера/калькулятора/дровера — «ПРОБЛЕМ ПЕРЕНОСА НЕТ»; admin 390px — агент, 0 overflow; break-words на comment/note/email.
- ТЕСТЫ: НОВЫЙ research/c102/A-tests.sh 38/38 (+1 SKIP дайджеста в 6ч МСК — время-зависим): C1-C2 контакты писём, E1-E3 защита cron, E4-E5 токен, E6-E9 дожим=1 письмо+PDF+MAX+идемпотентность, E10 CLI, E11-E12 статусы/заметки/валидация, E13-E14 настройки, E15 отзыв. c101 A-tests TG2/TG3 переписаны под 2-строчную клавиатуру (+lead_tg_href/lead_phone_digits) → 28/28. Регрессии: c100 A 59/59, SMTP 27/27, c99 A 56/56. php -l ×4, lint, tsc — чисто.
- docs/AUTOMATION.md — инструкция владельцу (SpaceWeb cron за 3 минуты, что где смотреть).

Stage Summary:
- TG-уведомление: «✈️ Написать в Telegram» открывает чат с КЛИЕНТОМ по номеру (WA — вторым, Меню — второй строкой).
- MAX: дровер сайта, письмо-подвал ВСЕХ писем, письмо клиенту (текст+HTML), дожим-письмо, (legacy contact/calculator — для будущих рефлексий), живые hacc-booking/футер уже были. Формат share проверен живым max.ru.
- АДМИНКА: мини-CRM (статусы/заметки/быстрые каналы/архив/фильтры/статистика/CSV) + «Автоматизация» с cron-токеном и состоянием.
- АВТОМАТИЗАЦИЯ: cron.php (дайджест каждое утро + авто-дожим клиента через 24 ч + опциональные напоминания) — владелец настраивает один раз в админке, дальше система работает сама; идемпотентность и анти-шторм заложены.
- ВСЕ ТЕСТЫ ЗЕЛЁНЫЕ; готово к коммиту/деплою (следующий шаг).
